...
function padCell (cellValue = '', padding, width) {
  const diff = stringWidth(cellValue) - cellValue.length
  return (padding.left || '') + cellValue.padEnd(width - padding.length() - diff) + (padding.right || '')
}

function getLongestWord (line) {
  const words = wrap.getChunks(line)
  return words.reduce((max, word) => Math.max(stringWidth(word), max), 0)
}
